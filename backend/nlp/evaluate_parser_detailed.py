...
# --- Initialize counters & mismatch log ---
...
for entry in data:
    ...
    for f in fields:
        ...
        if f == "interests":
            match = lists_match(pred_val, exp_val)
        elif f in ["start_date", "end_date"]:
            match = dates_close(pred_val, exp_val)
        elif f in ["trip_duration", "group_size", "budget"]:
            match = numbers_close(pred_val, exp_val)
        elif isinstance(exp_val, str) and isinstance(pred_val, str):
            match = exp_val.lower().strip() == pred_val.lower().strip()
        else:
...
if mismatches:
    ...
    df.to_csv("backend/nlp/data/parser_mismatches.csv", index=False, encoding="utf-8")
    print(f"\n⚠️  {len(mismatches)} mismatched entries saved to backend/nlp/data/parser_mismatches.csv")
else:
    print("\n✅ All queries matched perfectly!")
